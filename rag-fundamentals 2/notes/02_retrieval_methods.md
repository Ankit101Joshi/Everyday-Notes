...
## Diagram: End-to-End Retrieval Pipeline

```mermaid
flowchart TD
    A[User Query] --> B[Hybrid Search: Sparse + Dense]
    B --> C["Top-50 Candidates"]
    C --> D[Cross-Encoder Reranker]
    D --> E["Top-5 Precision Results"]
    E --> F[Context Packing + Citations]
    F --> G[LLM Generation]

    H[(Vector Index: HNSW/IVF)] --> B
    I[(BM25 Index)] --> B
```

## Interview Q&A Quick-Fire

**Q: What's the difference between recall and precision in retrieval?**
A: Recall is how much of the actually-relevant content the system is *capable* of finding at all; precision is how much of what it *did* return is genuinely relevant. Good ingestion drives recall; reranking drives precision.

**Q: What's the trade-off controlled by efSearch/nprobe?**
A: Recall vs. latency — searching more of the index (higher efSearch/nprobe) finds better matches but takes longer; lower values are faster but risk missing the true best matches.

**Q: Why use a two-stage pipeline (retrieve then rerank) instead of just retrieving more candidates?**
A: A cross-encoder reranker is far too slow to run over an entire corpus, so a fast/coarse method (vector or hybrid search) narrows millions of chunks to a shortlist first, and the expensive reranker only runs on that small shortlist.

**Q: How would you prevent a retrieval system from leaking data across tenants?**
A: Enforce ACL/tenant filtering as metadata attached at ingestion time, and apply that filter *at retrieval time* — not as a post-hoc check after results are already returned.

**Q: What are the four SLOs that matter for a retrieval pipeline, and how do they conflict?**
A: Latency, Freshness, Security, and Availability. They pull against each other — e.g., aggressive caching improves latency but hurts freshness; strict ACL filtering improves security but can add latency.

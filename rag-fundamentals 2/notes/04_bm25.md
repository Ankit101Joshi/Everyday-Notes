...
## Diagram: BM25 vs. TF-IDF on Repeated Terms

```mermaid
flowchart TD
    A["Word appears N times in a document"] --> B{Scoring Method}
    B -->|TF-IDF| C["Score grows ~linearly with N"]
    B -->|BM25| D["Score rises fast, then saturates via k1"]
    C --> E["Vulnerable to keyword stuffing"]
    D --> F["Resistant to keyword stuffing"]
```

## Interview Q&A Quick-Fire

**Q: What do the k1 and b parameters in BM25 control?**
A: `k1` controls how quickly term-frequency saturates (diminishing returns per repeated occurrence); `b` controls how strongly document length is normalized against the collection average (0 = no normalization, 1 = full normalization).

**Q: How does BM25 prevent long documents from unfairly winning?**
A: It divides a document's effective term frequency by a factor that grows with `docLen / avgDocLen` — so a document that's long simply because it's long, not because it's more relevant, gets its score pulled back down.

**Q: Where is BM25 actually used today?**
A: It's the default "sparse" retrieval method behind Elasticsearch, OpenSearch, and Postgres full-text search, and it's typically the keyword half of hybrid search in production RAG pipelines.

**Q: If BM25 is better than TF-IDF, why learn TF-IDF at all?**
A: TF-IDF is the conceptual foundation BM25 builds on (rare-and-frequent = important) — understanding it makes BM25's two fixes (saturation, length normalization) meaningful rather than arbitrary.

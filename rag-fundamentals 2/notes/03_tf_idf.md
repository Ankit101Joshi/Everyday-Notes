...
## Diagram: TF-IDF Scoring Pipeline

```mermaid
flowchart LR
    A[Document Collection] --> B[Tokenize each document]
    B --> C["Compute TF: word count / doc length"]
    B --> D["Compute IDF: log(N / docs containing word)"]
    C --> E["TF-IDF = TF × IDF"]
    D --> E
    E --> F[Rank documents by score]
```

## Interview Q&A Quick-Fire

**Q: Why does IDF use a logarithm instead of a raw ratio?**
A: A raw ratio (total docs / docs containing the word) can produce enormous numbers for very rare words — the log compresses that into a much more usable, less extreme scale.

**Q: Why does a word like "the" get a low TF-IDF score even if it appears often in a document?**
A: Its high TF is cancelled out by a near-zero IDF, since it appears in almost every document in the collection — TF-IDF rewards words that are both frequent *here* and rare *elsewhere*.

**Q: What's the main limitation of TF-IDF?**
A: It has no understanding of meaning, synonyms, or word order — "car" and "automobile" score as completely unrelated, and "dog bites man" scores identically to "man bites dog."

**Q: What replaced TF-IDF in most modern production systems?**
A: BM25 — it keeps TF-IDF's core idea but adds term-frequency saturation and document-length normalization, fixing TF-IDF's bias toward long or keyword-stuffed documents.

...
| 11 | [`11_vector_database_selection_guide.md`](notes/11_vector_database_selection_guide.md) | A practical buyer's guide — local vs. self-hosted vs. managed, a use-case decision tree, and the cost/scaling gotchas that don't show up in a feature comparison table. |
| 12 | [`12_reranking.md`](notes/12_reranking.md) | The precision stage after retrieval — cross-encoder scoring, why it's a separate stage, and when it's worth the extra latency. |
...
5. **Vector Databases** (#10) and the **Selection Guide** (#11) — "why would you pick tool X" and "walk me through the trade-offs" questions are common in system-design-style interviews, and #11's decision tree and cost gotchas are exactly what those questions are probing for.

...
## Diagram: Two Paths to the Same Query

```mermaid
flowchart TD
    Q[User Query: 'car repair'] --> K[Keyword Path: BM25]
    Q --> S[Semantic Path: Embedding + Vector Search]
    K --> K1["Matches: documents containing 'car' or 'repair'"]
    S --> S1["Matches: documents about 'automobile maintenance' too"]
    K1 --> H[Hybrid Combination]
    S1 --> H
    H --> R[Final Ranked Results]
```

## Interview Q&A Quick-Fire

**Q: Give an example where keyword search wins and semantic search loses.**
A: A search for an exact error code or product SKU — the exact string match matters, and an embedding model may not represent that rare token precisely enough to retrieve it reliably.

**Q: Give an example where semantic search wins and keyword search loses.**
A: A query like "how do I fix a slow laptop" matching a document titled "Improving computer performance" — almost no shared words, but the same underlying meaning.

**Q: Why do most production RAG systems use hybrid search instead of picking one?**
A: Because the two methods' failure modes are near-opposite — keyword search misses paraphrases, semantic search can miss exact rare terms — so combining both catches more genuinely relevant results than either alone.

**Q: Is hybrid search always the right choice?**
A: Not necessarily — it adds complexity and compute cost. For small, well-structured corpora or latency-critical systems, pure keyword or pure semantic search alone may be sufficient.

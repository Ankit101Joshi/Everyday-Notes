...
## Diagram: Cosine Similarity Visualized

```mermaid
flowchart LR
    A["Vector A"] -->|"angle = 0°"| B["Vector B (same direction)"]
    B --> S1["cos(θ) = 1 → Highly similar"]
    A -->|"angle = 90°"| C["Vector C (perpendicular)"]
    C --> S2["cos(θ) = 0 → Unrelated"]
    A -->|"angle = 180°"| D["Vector D (opposite)"]
    D --> S3["cos(θ) = -1 → Maximally dissimilar"]
```

## Interview Q&A Quick-Fire

**Q: Why is cosine similarity preferred over Euclidean distance for comparing text embeddings?**
A: Cosine similarity ignores vector magnitude and only measures direction, so a long and a short document about the same topic can still score as highly similar — Euclidean distance penalizes longer texts simply for having larger-magnitude vectors.

**Q: Walk through the cosine similarity formula.**
A: cos(θ) = (A · B) / (‖A‖ × ‖B‖) — the dot product of the two vectors, divided by the product of their magnitudes; dividing by magnitude is what removes the effect of vector length.

**Q: When might you deliberately use dot product instead of cosine similarity?**
A: When a model deliberately encodes meaningful information in vector magnitude (e.g., specificity or confidence) — in that case normalizing it away with cosine similarity would throw away a useful signal.

**Q: What does a cosine similarity score close to 0 tell you?**
A: The two pieces of text are essentially unrelated in meaning — their embedding vectors are close to perpendicular in the vector space.

...
## Diagram: The Four Failure Modes at a Glance

```mermaid
flowchart TD
    A[Keyword Search Query] --> B{Failure Mode?}
    B -->|Different words, same meaning| C[Lexical Mismatch]
    B -->|Same word, different meaning| D[Polysemy / Context Blindness]
    B -->|Term buried in a big doc| E[Passage Granularity]
    B -->|Typos / slang| F[Noisy Language]
    C --> G[Fix: Dense/Semantic Retrieval]
    D --> G
    E --> H[Fix: Better Chunking]
    F --> G
```

## Interview Q&A Quick-Fire

**Q: What is an inverted index, and why does keyword search need one?**
A: A data structure mapping each word to the list of documents containing it — it turns keyword search from a slow document-by-document scan into a fast direct lookup.

**Q: What's the difference between Lexical Mismatch and Polysemy?**
A: Lexical Mismatch is different words meaning the same thing (car/automobile) causing a missed match; Polysemy is the same word meaning different things (bank/river vs. bank/finance) causing a false match.

**Q: Why is "Passage Granularity" really a chunking problem, not just a retrieval problem?**
A: A keyword match at the whole-document level doesn't guarantee the matched *passage* is relevant — retrieving at the chunk level (proper chunking) is what makes relevance judgments precise instead of document-wide.

**Q: Name two mitigations for keyword search's weaknesses that don't require switching to vector search.**
A: Query expansion (adding synonyms to the query before search) and maintaining a curated synonym map for known important domain terms.

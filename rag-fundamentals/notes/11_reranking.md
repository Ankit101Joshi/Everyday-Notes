...
## Diagram: Reranking in the Full Pipeline

```mermaid
flowchart LR
    A[User Query] --> B["Initial Retrieval (Hybrid: BM25 + Vector)"]
    B --> C["Top-50 Candidates"]
    C --> D["Cross-Encoder Reranker"]
    D --> E["Top-5 High-Precision Results"]
    E --> F[LLM Context]
...
**Q: Why is reranking a separate stage instead of just making initial retrieval more accurate?**
A: Initial retrieval (bi-encoder/vector search) is optimized for speed over the whole corpus via precomputed embeddings; a cross-encoder is more accurate but too slow to run at that scale — reranking applies the expensive model only to a small shortlist.

**Q: What's the practical risk of skipping reranking entirely?**
A: The LLM's context window ends up filled with the initial retrieval's noisier top-K, including irrelevant or loosely-related chunks that can pull the generated answer off track.

**Q: In what situations would you skip reranking?**
A: Ultra-low-latency applications, small/clean corpora where initial retrieval is already precise, or early-stage prototypes where the added pipeline complexity isn't yet justified by evidence of a quality problem.

**Q: How does reranking relate to the dual-encoder/cross-encoder distinction?**
A: Reranking is precisely where the cross-encoder is used — dual-encoders (Sentence Transformers) power the fast initial retrieval, cross-encoders power the slow, precise reranking step immediately after.
